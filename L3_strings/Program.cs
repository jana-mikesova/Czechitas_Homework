...
it was the grayish white of old porridge.";
            ...
            int ab = string.Compare(blabol1,blabol2, StringComparison.OrdinalIgnoreCase);
            if (ab >= 0) 
            {
                int bc = string.Compare(blabol2,blabol3, StringComparison.OrdinalIgnoreCase);
                if ( bc >= 0)
                {
                    prvni = blabol3;
                } else 
            ...
            {
                int ac = string.Compare(blabol1, blabol3, StringComparison.OrdinalIgnoreCase);
                if (ac >= 0)
                {
                    prvni = blabol3;
                } else
            ...
            int vykricnikIndex = text.IndexOf("!");
            string pomocnyText = text.Substring(0, vykricnikIndex+1);
            
            int teckaIndex = pomocnyText.IndexOf(".");
            int otaznikIndex = pomocnyText.IndexOf("?");

            if (teckaIndex == -1 && otaznikIndex == -1)
            {
                veta = pomocnyText.Trim(' ','"','!','.','?');
            } else if ((teckaIndex >=0 && otaznikIndex == -1) || (teckaIndex > otaznikIndex))
            {
                veta = pomocnyText.Substring(teckaIndex).Trim(' ', '"', '!', '.', '?');
            } else 
            {
                veta = pomocnyText.Substring(otaznikIndex).Trim(' ', '"', '!', '.', '?');
            } 
            ...
            {
                pocetAnd++;
                indexAnd = pomocnyText.IndexOf(" and ");
                pomocnyText = pomocnyText.Substring(indexAnd + 5);
            }
